...
#define NUM_OF_MUTEXES 4
#define INDEX_FOR_MUTEX_FOR_READY 3
#define INDEX_OF_MUTEX_FOR_MAIN_THREAD 0
#define INDEX_OF_MUTEX_FOR_NEW_THREAD 2
...
void *print_n_str(void *arg) {
    ...
    while (str_number < args->num_of_str) {
        int next_index_of_mutex = (index_of_using_mutex + 1) % (NUM_OF_MUTEXES - 1);
        pthread_mutex_lock(&mutexes[next_index_of_mutex]);
	printf("%s with %d number of string\n", args->message, str_number);
        pthread_mutex_unlock(&mutexes[index_of_using_mutex]);
        index_of_using_mutex = next_index_of_mutex;
        ++str_number;
    }
    pthread_mutex_unlock(&mutexes[index_of_using_mutex]);
    return NULL;
...
void *thread_function(void *arg) {
    struct print_args *args = (struct print_args *)arg;
    pthread_mutex_lock(&mutexes[INDEX_FOR_MUTEX_FOR_READY]);
    pthread_mutex_lock(&mutexes[args->initial_mutex_index_for_thread]);
    ++num_of_ready_threads;
    pthread_mutex_unlock(&mutexes[INDEX_FOR_MUTEX_FOR_READY]);
    while (true) { 
        pthread_mutex_lock(&mutexes[INDEX_FOR_MUTEX_FOR_READY]);
...
int main() {
    ...
    return_code = pthread_create(&thread_id, NULL, thread_function, (void *)&args_for_child);
    if (return_code != SUCCESS_CODE) {
    ...
    thread_function((void *)&args_for_parent);
